...
function [labeledData,annos,captionName2annosIDs,annosID2captionName,Dictionaries1]=read_STA_othermods(fileAnNLP,fileAnST,Dictionaries)


[annos,captionName2annosIDs,annosID2captionName]=read_NLP_Pipeline_Output_withid_othermods(fileAnNLP);
fid=fopen(fileAnST);
line=fgets(fid);
l=0;
...
while line ~= -1
    ...
    where = find(strcmp(test(2:end),ids));
    if length(where)>1;
        'hello'
    end
    tree=annos(where);
